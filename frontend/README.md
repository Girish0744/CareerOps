...
| `/applications/[id]` | Application Detail | Resume, cover letter, chat editor, interview guide, outreach drafts |
...
- Scanner default freshness mode is `scan.freshnessWindowHours: 24`; scan queues rank recent postings first, then full-time new-grad/entry-level role priority, then score and source quality.
...
- Job Discovery scan cards are grouped into Strong Apply / Apply / Maybe / Skip lanes and can be filtered by score, company, source, role type, and freshness (`24h`, `72h`, `7d`, `all`).
...
- Validation commands for the current baseline: root `npm run scan:qa`, `npm run eval:qa`, `npm run contacts:qa`, `node doctor.mjs`, `node cv-sync-check.mjs`, `node verify-pipeline.mjs`; frontend `npm run lint` and `npm run build`.
