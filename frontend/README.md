...
| `/api/applications/[id]/apply/current-tab` | POST | Browser extension bridge: saves a safe current-tab Apply link or existing application-form URL into the application session for audit. |
...
| 17C | **Chrome extension companion** — MV3 current-tab companion fills high-confidence fields on the active employer/ATS tab, opens safe posting-page Apply links when needed, saves the apply URL for audit, and keeps the final-submit guard mandatory. | ✅ First foundation | `browser-extension/`, `app/api/applications/[id]/apply/current-tab` |
