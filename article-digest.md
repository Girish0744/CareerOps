...
- LLM assistant answering scenario questions from the platform's own census, competitor, and model output as context. VERIFIED against the codebase (github.com/Shubham231004/BestSpot.biz, backend/app/services/ai_*.py) on 2026-08-27: the scenario snapshot is serialized to JSON and injected into the prompt. There is NO embedding model, vector store, or similarity search anywhere in the repo, so this is context grounding, NOT RAG. Never claim RAG or vector retrieval.
- What IS true and claimable, all verified in that code: runs on Ollama with a hosted-provider fallback; a model allowlist rejects arbitrary model names; untrusted chat history and user questions are fenced and fence markers stripped, with explicit anti-prompt-injection instructions; the system prompt forbids inventing competitors, listings or revenue guarantees and forces the answer to separate observed census inputs from model predictions, proxy estimates and limitations; a pytest suite covers the allowlist and the injection fencing.
